    ...
    <v-footer dark color="primary footer-block" align="center" justify="center">
                ...
                <v-col cols="12" lg="3">
                    ...
                    <ul>
                        <li v-for="service in serviceArray[0]" :key="service.id">
                            <nuxt-link :to="`/${language.lang}/services/${service.id}`">{{ service.translation.name }}</nuxt-link>
                        </li>
                    </ul>
                </v-col>
                <v-col cols="12" lg="3">
        ...
        <!-- <v-card
            ...
            <v-row justify="center" no-gutters>
                <v-btn color="white" text rounded class="my-2" v-for="page in pages" :key="page.id" :to="`/${language.lang}/${page.alias}`">
                    {{ page.translation.name }}
                </v-btn>
            </v-row>
            <v-card-text class="white--text pt-0">
                {{ trans.HeaderFooter.footerText }}
            </v-card-text>
            <v-divider></v-divider>
            <v-card-text class="white--text">
                {{ new Date().getFullYear() }} — <strong>Likemedia</strong>
            </v-card-text>
        </v-card> -->
    </v-footer>
</template>
